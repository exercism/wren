...
Testie.test("Flatten Array") { |do, skip|
  ...
  skip.test("consecutive null values at the front of the list are omitted from the final result") {
    Expect.value(Flatten.flatten([null, null, 3])).toEqual([3])
  }

  skip.test("consecutive null values in the missle of the list are omitted from the final result") {

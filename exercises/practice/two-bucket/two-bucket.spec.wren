...
Testie.test("Two Bucket") { |do, skip|
  ...
  skip.test("Measure using bucket one of size 3 and bucket two of size 5 - start with bucket two") {
  ...
  skip.test("Measure using bucket one of size 7 and bucket two of size 11 - start with bucket one") {
  ...
  skip.test("Measure using bucket one of size 7 and bucket two of size 11 - start with bucket two") {
  ...
  skip.test("With the same buckets but a different goal, then it is possible") {
  ...
  skip.test("Goal larger than both buckets is impossible") {
    Expect.that {

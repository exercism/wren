...
Testie.test("ISBN Verifier") { |do, skip|
  ...
  skip.test("invalid check digit in isbn is not treated as zero") {
  ...
  skip.test("invalid characters are not ignored after checking length") {
    Expect.value(ISBN.new("3132P34035").isValid).toEqual(false)
  }

  skip.test("invalid characters are not ignored before checking length") {

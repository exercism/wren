...
Testie.test("Collatz.steps()") { |do, skip|
  do.test("zero steps for one") {
    Expect.that(Collatz.steps(1)).toEqual(0)
  }

  skip.test("divide if even") {
    Expect.that(Collatz.steps(16)).toEqual(4)
  }

  skip.test("even and odd steps") {
    Expect.that(Collatz.steps(12)).toEqual(9)
  }

  skip.test("large number of even and odd steps") {
    Expect.that(Collatz.steps(1000000)).toEqual(152)
  }

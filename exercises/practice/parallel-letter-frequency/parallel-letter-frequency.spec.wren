import "./parallel-letter-frequency" for ParallelLetterFrequency
import "wren-testie/testie" for Testie, Expect

Testie.test("ParallelLetterFrequency") { |do, skip|
  do.test("no texts") {
    var input = []
    var actual = ParallelLetterFrequency.new(input).calculateFrequencies()
    var expected = {}
    Expect.value(actual).toEqual(expected)
  }

  skip.test("one text with one letter") {
    var input = ["a"]
    var actual = ParallelLetterFrequency.new(input).calculateFrequencies()
    var expected = {
      "a": 1
  ...
  skip.test("one text with multiple letters") {
    var input = ["bbcccd"]
    var actual = ParallelLetterFrequency.new(input).calculateFrequencies()
    var expected = {
      "b": 2,
  ...
  skip.test("two texts with one letter") {
    var input = ["e", "f"]
    var actual = ParallelLetterFrequency.new(input).calculateFrequencies()
    var expected = {
      "e": 1,
  ...
  skip.test("two texts with multiple letters") {
    var input = ["ggh", "hhi"]
    var actual = ParallelLetterFrequency.new(input).calculateFrequencies()
    var expected = {
      "g": 2,
  ...
  skip.test("ignore letter casing") {
    var input = ["m", "M"]
    var actual = ParallelLetterFrequency.new(input).calculateFrequencies()
    var expected = {
      "m": 2
  ...
  skip.test("combination of lower- and uppercase letters, punctuation and white space") {
    var input = [
